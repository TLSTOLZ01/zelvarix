      ...
      <div style={{ minHeight:"100vh", background:T.cream, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'DM Sans',sans-serif" }}>
        ...
        <div style={{ width:480, animation:"fadeIn .4s ease" }}>
          ...
          <div style={{ background:"#fff", border:`1px solid ${T.border}`, borderRadius:6, padding:"32px 32px 28px", boxShadow:`0 2px 16px ${T.shadow}` }}>
            ...
            <button onClick={()=>onboardStep<onboardSteps.length-1?setOnboardStep(s=>s+1):setAppView("app")} onClick={async()=>{ if(onboardStep===onboardSteps.length-1 && currentUser){ await sb.from("profiles").upsert({ id:currentUser.id, name:onboardData.name, company:onboardData.company, role:onboardData.role, goal:onboardData.goal }); } }} style={{ width:"100%", padding:"11px", background:T.ink, border:"none", borderRadius:4, color:T.cream, fontWeight:600, fontSize:14, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>
